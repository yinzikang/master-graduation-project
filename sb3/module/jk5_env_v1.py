...
class TrainEnv(Jk5StickRobotEnv):
    ...
    def GetReward(self, done, observation, action, x_error, xd_error, contact_force, nft_force, tau):
        """
        奖励组成包括：对每一个运动状态的奖励、任务结束的奖励与惩罚、
        - 运动状态的奖励：关于接触力、位置、速度、力矩、刚度
        - 任务结束的奖励与惩罚：关于当前步数
        - 接触力的奖励与惩罚：关于特别大的接触力，以及不接触情况
        每一时刻的奖励不宜超过[-1,1]
        :param done:
        :param observation:
        :param action:
        ...
        fext_reward = (20 - np.sum(
            np.linalg.norm(contact_force - self.desired_force_list[self.current_step], ord=1))) / 20
        tau_reward = - np.sqrt(np.sum((tau - self.last_tau) ** 2))
        error_k_penalty = -1 if (self.K > self.max_K).any() or (self.K < self.min_K).any() else 0
        # 任务结束的奖励与惩罚
        ...
        return fext_reward + \
            error_k_penalty + \
            early_stop_penalty + \
            0.1 * zero_force_penalty + \
            massive_force_penalty
    ...
    def step(self, action):
        ...
        # 刚度越界或者任务结束，视为done
        success = True if self.current_step + 1 == self.step_num else False
        failure = True if ((self.K > self.max_K).any() or (self.K < self.min_K).any()) else False
        done = success or failure
        other_info = None
        # 计算该(observation,action)获得的奖励
        reward = self.GetReward(done, observation, action, x_error, xd_error, contact_force, nft_force, tau)
        ...
        return next_observation, reward, done, other_info
...
def load_env_kwargs(task=None):
    if task == 'desk':
        ...
        desired_force_list = np.array([[0, 0, 20, 0, 0, 0]], dtype=np.float32).repeat(step_num, axis=0)
        # 阻抗参数
        ...
        env_kwargs = dict(mjc_model_path=mjc_model_path, task=task, qpos_init_list=qpos_init_list,
                          p_bias=p_bias, r_bias=r_bias, step_num=step_num,
