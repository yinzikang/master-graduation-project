...
from gym_custom.envs.controller import trajectory_planning_line, trajectory_planning_circle, \
    ComputedTorqueController, ImpedanceController, AdmittanceController, ImpedanceController_v2, AdmittanceController_v2
...
def env_kwargs(task=None, save_flag=False, save_path=None):
    ...
    if task == 'test':
        ...
        K = np.array([1000, 2000, 3000, 1000, 1000, 1000], dtype=np.float64)
        M = K / (wn * wn)
