...
def joint_interpolation(init_qpos, end_qpos, time_whole, time_acceleration, control_frequency):
    """
    返回step_num*3
    :param init_qpos:
    :param end_qpos:
    ...
    """
    step_num = int(time_whole * control_frequency)
    error = end_qpos - init_qpos  # 直线运动，误差为位置的差距
    a, v, p = get_trajectory_para(time_whole, time_acceleration, step_num)
    acc_buffer = a * error
    vel_buffer = v * error
    pos_buffer = p * error + init_qpos
...
# 用来画图
class ImpedanceController_v3:
    def __init__(self, orientation_error=orientation_error_quat_with_mat):
    ...
    def step(self, status):
        ...
        xposture_error = np.concatenate([desired_xpos - xpos,
                                         self.orientation_error(desired_xmat, xmat)])

        xvel_error = desired_xvel - xvel
        # print(K_M @ xposture_error + contact_force)
        # 阻抗控制率
        T = B_M @ xvel_error + K_M @ xposture_error + contact_force
        solved_acc = desired_xacc + np.linalg.inv(M_M) @ T
        F = D_x @ solved_acc + CG_x - contact_force
...
class AdmittanceController_v4:
    ...
    def step(self, status):
        ...
        desired_xvel = status['desired_xvel']
        desired_xacc = status['desired_xacc']
        desired_force = status['desired_force']
        contact_force = status['contact_force']
        ...
        if np.max(np.abs(contact_force)) < 1e-5:
            print("free space")
        else:
            print("contact space")
            K[2] = 0
        ...
        xposture_error = np.concatenate([desired_xpos - self.compliant_xpos,
                                         orientation_error_quat_with_quat(desired_xquat, self.compliant_xquat)])
        xvel_error = desired_xvel - self.compliant_xvel
        T = B_M @ xvel_error + K_M @ xposture_error + contact_force - desired_force
        solved_acc = desired_xacc + np.linalg.inv(M_M) @ T  # compliant_xacc
        self.compliant_xacc = solved_acc
        self.compliant_xvel += self.compliant_xacc * timestep
        self.compliant_xpos += self.compliant_xvel[:3] * timestep
